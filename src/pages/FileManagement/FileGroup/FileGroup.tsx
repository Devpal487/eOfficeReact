...
export default function FileGroup() {
            ...
            if (data && data.length > 0) {
                ...
                if (userPermissionData && userPermissionData.length > 0) {
                    ...
                    for (let index = 0; index < menudata.length; index++) {
                        ...
                        const PMenudata = menudata[index]?.menuName;
                        console.log(childMenudata)
    ...
    const accept = () => {
        const collectData = {
            fileGroupid: delete_id,
        };
        console.log("collectData " + JSON.stringify(collectData));
        api
            .delete(`FileGroup/DeleteFileGroup`, {data: collectData})
            .then((response) => {
    ...
    const getList =  () => {
        ...
        try {
             api
                .post(`FileGroup/GetFileGroup`, collectData)
                .then((res) => {
                    ...
                    if (data.length > 0) {
                        const columns: GridColDef[] = [
                            ...
                            {
                                field: "fileGroupName",
                                headerName: t("text.FileGroupName"),
                                flex: 1,
                                headerClassName: "MuiDataGrid-colCell",
                            },
                        ];
    ...
    const validationSchema = Yup.object({
        fileGroupName: Yup.string().test(
            "required",
            t("text.reqFileGroupName"),
            function (value: any) {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            values.fileGroupid = editId;
            // console.log("check", values);

            const response = await api.post(
                `FileGroup/AddUpdateFileGroup`,
                values
            );
            if (response.data.isSuccess) {
                setToaster(false);
    ...
    const requiredFields = ["fileGroupName"];


    const routeChangeEdit = (row: any) => {
        formik.setFieldValue("fileGroupName", row.fileGroupName);
        setEditId(row.id);
    };
