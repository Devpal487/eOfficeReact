...
export default function FileSubject() {
    ...
    const accept = () => {
        ...
        api
            .delete( `FileSubject/DeleteFileSubject`, {data:collectData})
            .then((response) => {
    ...
    const getList = () => {
        ...
        try {
            api
                .post( `FileSubject/GetFileSubject`, collectData)
                .then((res) => {
                    ...
                    if (data.length > 0) {
                        const columns: GridColDef[] = [
                            ...
                            {
                                field: "fileSubjectName",
                                headerName: t("text.FileSubjectName"),
                                flex: 1,
                                headerClassName: "MuiDataGrid-colCell",
                            },
                            {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
           values.id = editId;
            ...
            const response = await api.post( `FileSubject/AddUpdateFileSubject`,
                values
            );
            if (response.data.isSuccess) {
                setToaster(false);
    ...
    const requiredFields = ["fileSubjectName"];


    const routeChangeEdit = (row: any) => {
        formik.setFieldValue("fileSubjectName", row.fileSubjectName);
