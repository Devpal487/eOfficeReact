...
export default function CourseMaster() {
  ...
  const accept = () => {
    ...
    api
      .delete(`CourseMaster/DeleteCourseMaster`, { data: collectData })
      .then((response) => {
  ...
  const getList = () => {
    ...
    try {
      api.post(`CourseMaster/GetCourseMaster`, collectData).then((res) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      values.courseId = editId;

      const response = await api.post(`CourseMaster/AddUpdateCourseMaster`, values);
      if (response.data.isSuccess) {
        setToaster(false);
        toast.success(response.data.mesg);
        formik.resetForm();
  ...
  const routeChangeEdit = (row: any) => {
    formik.setFieldValue("courseName", row.courseName);
    formik.setFieldValue("shortName", row.shortName);
    formik.setFieldValue("courseId", row.courseId);
    setEditId(row.id);
  };
          ...
          >
            ...
            <form onSubmit={formik.handleSubmit}>
              <Grid item xs={12} container spacing={2}>
                <Grid item xs={5}>
                  <TranslateTextField
                    label={t("text.EnterCourseName")}
                    value={formik.values.courseName}
                    onChangeText={(text: string) =>
                      handleConversionChange("courseName", text)
                    }
                    required={true}
                    lang={lang}
                  />
